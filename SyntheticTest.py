...
Modified 2015 Paul Buis, Ball State University
"""

#
# fp = false_positives -> words identified as misspelled that were correctly spelled
# fn = false_negatives -> words identified as correctly spelled that were not
# tp = true_positives -> words identified as misspelled that were misspelled
# tn = true_negatives -> words identified as correctly spelled that were correctly spelled
#
# n = tp + fp + tn + fn
# accuracy = (tp + tn) / n
# R = recall = tp / (tp + fn)    ??? See Jurasky paper
# P = precision = tp / (tp + fp) ??? See Jurasky paper
# F1 score = 2*P*R / (P+R)


def spell_test(correct, NWORDS, tests, bias=None, verbose=False):
    import time
    ...
    return dict(bad=bad, n=n, bias=bias, pct=int(100. - 100.*bad/n),
                ...
                speed=(time.clock()-start)/(tn+tp+fn+fp))
